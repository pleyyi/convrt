...
function checkForUpdate() {
    chrome.runtime.sendMessage({ action: 'checkUpdate' }, function(resp) {
        ...
        if (resp.tag !== CURRENT_VERSION) {
            ...
            text.textContent = 'update available: ' + resp.tag;
            link.href = resp.url || 'https://github.com/pleyyi/convrt/releases/latest';
...
document.getElementById('clearHistBtn').addEventListener('click', function () {
    testHistory = [];
    chrome.storage.local.set({ testHistory: [] }, renderHistory);
});
